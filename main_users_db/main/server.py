...
class UsersCBV:
    ...
    async def user_registration(self, user: RegistrationValidation) -> JSONResponse:
        ...
        user, access_token, refresh_token = await asyncio.gather(
            ...
            self.token.create_access_token(user.usename),
            self.token.create_refresh_token(user.usename)
        )
    ...
    async def update_user(self, request: Request, data: UserUpdateRequest):
    ...
    async def authentication(self, request_user: AuthenticationValidation) -> JSONResponse:
        ...
        self.token.sub = db_user.username
    ...
    async def current_user(self, request: Request, sub=None):
